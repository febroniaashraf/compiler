...
/**-----------------------------------------------------------------------------------------
 * USEAGE : checks if input word is included in language
        ...
        while(getline(newfile, line))  //read data from file object and put it into string.
        {
            ...
            for (std::string::size_type i = 0; i < line.size(); i++)
            {
                if(line[i] == ' ')
                {
                    if(word != "")
                    {
                        output.push_back(pair<string,string>(word,construct_output(word, mini)));
                        word = "";
                    }
                }
                else
...
/**-----------------------------------------------------------------------------------------
 * USEAGE : Write result in the output file
 * Take   : Vector of token classes and the attribute values
 *-----------------------------------------------------------------------------------------*/

void writeOutput(vector<pair<string,string>> output)
{
    std::ofstream outfile;
    string file_path = __FILE__;
    string dir_path = file_path.substr(0, file_path.rfind("\\"));
    string dir_path2 = dir_path.substr(0, dir_path.rfind("\\"));
    outfile.open(dir_path2+"\\output_phase1.txt", std::ios_base::trunc); //  overwrite
